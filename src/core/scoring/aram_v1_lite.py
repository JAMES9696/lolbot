...
def calculate_aram_build_adaptation(
    ...
    def classify_threat(ratio: float) -> str:
        if ratio >= 0.6:
            return "high"
        elif ratio >= 0.35:
            return "medium"
    ...
    enemy_ap_threat = classify_threat(ap_threat_ratio)
    enemy_ad_threat = classify_threat(ad_threat_ratio)
...
def generate_aram_analysis_report(
    ...
    # Match result
    match_result = "victory" if player_data["win"] else "defeat"

...
class UserBinding(BaseModel):
    ...
    discord_id: str = Field(..., description="Discord User ID (snowflake)", pattern=r"^\d{17,20}$")

    puuid: str | None = Field(
        None, description="Riot PUUID (Player Universally Unique ID)", min_length=78, max_length=78
    )

    summoner_name: str | None = Field(
        None, description="Summoner name (game name)", min_length=3, max_length=16
    )
    ...
    status: BindingStatus = Field(BindingStatus.PENDING, description="Current binding status")

    created_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC), description="Binding creation timestamp"
    )
    ...
    verification_token: str | None = Field(None, description="Temporary token for RSO verification")
...
class BindingRequest(BaseModel):
    ...
    region: str = Field("na1", description="Preferred region")
...
class BindingResponse(BaseModel):
    ...
    auth_url: str | None = Field(None, description="RSO authorization URL if applicable")

    binding: UserBinding | None = Field(None, description="User binding data if available")
...
class RiotAccount(BaseModel):
    ...
    puuid: str = Field(..., description="Player Universally Unique ID")

    game_name: str = Field(..., description="Riot ID game name")

    tag_line: str = Field(..., description="Riot ID tag line")

    summoner_id: str | None = Field(None, description="Summoner ID for the region")

    account_id: str | None = Field(None, description="Account ID")

    profile_icon_id: int | None = Field(None, description="Profile icon ID")

    summoner_level: int | None = Field(None, description="Summoner level")

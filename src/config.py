...
class Settings(BaseSettings):
    ...
    discord_guild_id: str | None = Field(
        None,
        description="Optional: Guild ID for development testing",
    ...
    riot_region: str = Field(
        "na1",
        description="Default Riot API region",
        alias="RIOT_REGION"
    )
    ...
    riot_client_id: str | None = Field(
        None,
        description="Riot OAuth Client ID for RSO",
        alias="RIOT_CLIENT_ID"
    )

    riot_client_secret: str | None = Field(
        None,
        description="Riot OAuth Client Secret for RSO",
        alias="RIOT_CLIENT_SECRET"
    )

    riot_redirect_uri: str | None = Field(
        None,
        description="OAuth redirect URI for RSO callback",
        alias="RIOT_REDIRECT_URI"
    )

    # Bot Configuration
    bot_prefix: str = Field(
        "!",
        description="Command prefix for text commands (if any)",
        alias="BOT_PREFIX"
    )
